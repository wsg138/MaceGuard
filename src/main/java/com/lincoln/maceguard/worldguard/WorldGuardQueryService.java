...
public final class WorldGuardQueryService {
    ...
    public boolean buildAllowed(Location location, Player player) {
        ...
        return query().testBuild(BukkitAdapter.adapt(location), player == null ? null : WorldGuardPlugin.inst().wrapPlayer(player));
    }
    ...
    public List<ProtectedRegion> applicableRegions(Location location) {
        ...
        return query().getApplicableRegions(BukkitAdapter.adapt(location)).getRegions().stream()
                .sorted(Comparator.comparingInt(ProtectedRegion::getPriority).reversed().thenComparing(ProtectedRegion::getId)).toList();
    }

    private boolean state(Location location, Player player, StateFlag flag) {
        if (flag == null || location.getWorld() == null) return false;
        StateFlag.State value = query().queryState(BukkitAdapter.adapt(location), player == null ? null : WorldGuardPlugin.inst().wrapPlayer(player), flag);
        return value == StateFlag.State.ALLOW;
    }

    private RegionQuery query() { return WorldGuard.getInstance().getPlatform().getRegionContainer().createQuery(); }
}
