...
public final class TemporaryBlockRepository {
    ...
    public Map<String, TemporaryBlock> load() throws IOException {
        ...
        try (Reader reader = Files.newBufferedReader(file, StandardCharsets.UTF_8)) {
            Map<String, TemporaryBlock> result = gson.fromJson(reader, new TypeToken<Map<String, TemporaryBlock>>() { }.getType());
            return result == null ? new LinkedHashMap<>() : new LinkedHashMap<>(result);
        } catch (RuntimeException ex) { throw new IOException("Invalid temporary block state", ex); }
    }
    ...
    public void save(Map<String, TemporaryBlock> blocks) throws IOException {
        ...
        try {
            try (Writer writer = Files.newBufferedWriter(temp, StandardCharsets.UTF_8)) { gson.toJson(blocks, writer); }
