...
public final class MaceGuardCommand implements CommandExecutor, TabCompleter {
        ...
        switch (sub) {
            ...
            case "filler", "restore" -> {
                if (args.length < 3 || (!args[2].equalsIgnoreCase("on") && !args[2].equalsIgnoreCase("off")))
                    sender.sendMessage("Usage: /maceguard " + sub + " <region> <on|off>");
                else if (args[2].equalsIgnoreCase("on")) plugin.runtime().resets().arm(world, region, sender::sendMessage);
            ...
            case "schedule" -> {
                ...
                else plugin.runtime().resets().setSchedule(world, region, args[2].equalsIgnoreCase("on"), sender::sendMessage);
            }
    ...
    private boolean here(CommandSender sender) {
        ...
        sender.sendMessage("Explosives: " + (query.explosivesDenied(location, player) ? "DENY" : "not denied by MaceGuard"));
    ...
    private void help(CommandSender sender) { sender.sendMessage("/maceguard here|status <region>|capture <region>|validate <region>|plan <region>|arm <region>|disarm <region>|filler <region> <on|off>|schedule <region> <on|off>|reset <region> <token>|recover <region>|temporary|reload"); }
    @Override public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) return List.of("here", "status", "capture", "validate", "plan", "arm", "disarm", "filler", "restore", "schedule", "reset", "recover", "temporary", "reload");
        if (args.length == 3 && (args[0].equalsIgnoreCase("filler") || args[0].equalsIgnoreCase("restore") || args[0].equalsIgnoreCase("schedule"))) return List.of("on", "off");
        return List.of();
    }
}
