...
- compatible modifier display names, descriptions, effects, restrictions, and start/end/warning messages;
...
Existing modifiers receive default `enabled` and `weight` values unless those fields already existed. New Pearl and Wind Charge outcomes, their restriction targets, and their conflict groups are added from the bundled schema.
...
5. Existing safe messages and modifier text remain present.
6. Every enabled modifier has a positive weight.
7. Count weights and Elytra percentages match operator intent.
8. The current selected IDs are all enabled and conflict-free.
9. The weekly boundary and transition were not shifted by a required reroll.
10. No reset profile was captured, armed, or scheduled.
